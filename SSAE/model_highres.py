...
class Encoder(nn.Module):
    """编码器：支持高分辨率输入（256x256或400x400）"""
    def __init__(self, input_channels=3, latent_dim=20, image_size=256):
        ...
        if image_size == 64:
            ...
            self.fc_size = 512 * 12 * 12  # 400->200->100->50->25->12.5->12
            self.num_layers = 6
        else:
...
class Decoder(nn.Module):
    """解码器：支持高分辨率输出"""
    def __init__(self, latent_dim=20, output_channels=3, image_size=256):
        ...
        if image_size == 64:
            ...
            self.deconv6 = nn.ConvTranspose2d(32, output_channels, kernel_size=4, stride=2, padding=1)
            self.num_layers = 6
        else:
            raise ValueError(f"不支持的图像尺寸: {image_size}")
